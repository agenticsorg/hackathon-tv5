...
import { MOCK_MOVIES, MOCK_TV_SHOWS } from './mock-tmdb';

// Environment validation
const TMDB_ACCESS_TOKEN = process.env.NEXT_PUBLIC_TMDB_ACCESS_TOKEN;

const IS_MOCK_MODE = !TMDB_ACCESS_TOKEN;

if (IS_MOCK_MODE) {
  console.warn('TMDB_ACCESS_TOKEN is not defined. Running in MOCK MODE.');
}
...
export const getImageUrl = (path: string | null, size: 'w500' | 'w780' | 'original' = 'w500'): string | null => {
  if (!path) return null;
  // If in mock mode and path is relative, return as is (assuming it might be a placeholder)
  // But for real TMDB paths, we prepend base
...
export async function searchMulti(
  ...
  if (IS_MOCK_MODE) {
    ...
    return { results: filtered, totalPages: 1, totalResults: filtered.length };
  }
...
export async function getPopularMovies(page = 1): Promise<{ results: Movie[]; totalPages: number }> {
  if (IS_MOCK_MODE) {
    return { results: MOCK_MOVIES, totalPages: 1 };
  }
...
export async function getPopularTVShows(page = 1): Promise<{ results: TVShow[]; totalPages: number }> {
  if (IS_MOCK_MODE) {
    return { results: MOCK_TV_SHOWS, totalPages: 1 };
  }
...
export async function getFullMovieDetails(id: number): Promise<{
  ...
  if (IS_MOCK_MODE) {
    ...
    return {
      ...
      credits: { cast: [], crew: [] },
      videos: [],
      similar: MOCK_MOVIES.slice(0, 2),
      recommendations: MOCK_MOVIES.slice(0, 2),
      genres: [],
    };
  }
...
export async function getFullTVShowDetails(id: number): Promise<{
  ...
  if (IS_MOCK_MODE) {
    const show = MOCK_TV_SHOWS.find(s => s.id === id) || MOCK_TV_SHOWS[0];
    return {
      show,
      credits: { cast: [], crew: [] },
      videos: [],
      similar: MOCK_TV_SHOWS.slice(0, 2),
      recommendations: MOCK_TV_SHOWS.slice(0, 2),
      genres: [],
    };
  }
 ...
 * Get watch providers
 */
export async function getWatchProviders(id: number, type: 'movie' | 'tv'): Promise<{
  ...
  if (IS_MOCK_MODE) {
    return {
      flatrate: [
        { provider_id: 8, provider_name: 'Netflix', logo_path: '/t2yyOv40HZeVlLkYsCsPHnWLk4W.jpg' },
        { provider_id: 119, provider_name: 'Amazon Prime Video', logo_path: '/emthp39XA2YScoU8vk5kAFagel2.jpg' },
      ],
      rent: [],
      buy: [],
  ...
  // Direct fetch as tmdb-ts might not have providers fully typed or accessible easily
  const response = await fetch(
  ...
  const data = await response.json();
  // Default to US for now, or use a locale if available
  const results = data.results?.US || data.results?.FR || {};
