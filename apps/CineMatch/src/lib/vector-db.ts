...
export async function getContentEmbedding(text: string): Promise<Float32Array | null> {
  try {
    ...
    if (OPENAI_API_KEY) {
      console.log(`🔄 Generating embedding (OpenAI) for: "${text.slice(0, 30)}..."`);
      ...
      if (!response.ok) {
        if (response.status === 401) {
          console.warn('OpenAI API key invalid (401), trying fallback...');
        } else {
    ...
    // Fallback to Gemini
    if (GOOGLE_API_KEY) {
      console.log(`🔄 Generating embedding (Gemini) for: "${text.slice(0, 30)}..."`);
      try {
        ...
        cacheEmbedding(cacheKey, float32Embedding);
    ...
    console.warn('No valid API keys found, using mock embedding');
  ...
  return filtered.slice(0, k).map((result: SearchResultItem) => {
    ...
    return {
      content: {
        ...
        voteCount: 0,
        popularity: 0,
        originalLanguage: meta.originalLanguage || 'en',
      },
      score: result.score,
    };
  });
